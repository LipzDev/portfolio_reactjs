...
const Particles = () => {
  ...
  function draw() {
    ...
    canvas.onmouseleave = particles.mouseHandler.bind(particles, {
      clientX: -100,
      clientY: -100,
    });
  }
  ...
  var particles = {
    ...
    mountParticles: function () {
      for (var i = 0; i < this.maxParticles; i++) {
        ...
        var speedX =
          Math.random() * 2 * Math.pow(-1, Math.floor(Math.random() * 10));
        var speedY =
          Math.random() * 2 * Math.pow(-1, Math.floor(Math.random() * 10));
  ...
  Particle.prototype.drawLine = function (distanceToMouse) {
    ...
    this.ctx.strokeStyle =
      'rgba(255, 255, 255, ' + (1 - distanceToMouse / 100).toFixed(1) + ')';
    this.ctx.stroke();
  };
